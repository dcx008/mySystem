...
Ext.ux.form.LovCombo = Ext.extend(Ext.form.ComboBox, {
    ...
    initComponent: function() {  
       ...
       this.tpl = ['<tpl for=".">',
					'<div class="x-combo-list-item">',
					'<div class="ux-combo-selectall-icon ux-lovcombo-iconux-lovcombo-icon-{[values.' + this.checkField + '?"checked":"unchecked"]}"',
    ...
    //     //这个方法本来有 但是没有用到 我在init 方法中添加了 beforeload  使用了此方法，但是下面这个方法 没有用到 所以暂时注释掉
    ...
    reDoResize : function(){
        ...
        if(ByUtil.isUE(this.maxCssLength)){
            ...
            this.getStore().each(function (rec) {
                if(maxLength<rec.get(showName).getCssLength()){
                    maxLength=rec.get(showName).getCssLength();
                }
            })
            //添加冗余宽度
